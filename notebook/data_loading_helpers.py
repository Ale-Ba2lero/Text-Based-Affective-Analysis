...
def extract_word_level_data(data_container, word_objects, eeg_float_resolution = np.float16):
    ...
    if isinstance(available_objects[0], str):
        ...
        if "rawEEG" in available_objects:
            ...
            psData = word_objects['meanPupilSize']
            try:
            ...
            zipped_data = zip(rawData, etData, contentData, ffdData, gdData, gptData, trtData, sfdData, psData, nFixData, fixPositions)
            word_level_data = {}
            word_idx = 0
            for raw_eegs_obj, ets_obj, word_obj, ffd, gd, gpt, trt, sfd, pS, nFix, fixPos in zipped_data:
                ...
                if is_real_word(word_string):
                    ...
                    data_dict["meanPupilSize"] = data_container[pS[0]][0, 0] if len(data_container[pS[0]].shape) == 2 else None
...
def extract_word_for_analysis(data_container, word_objects, eeg_float_resolution = np.float16):
    """
    Extracts word level data for a specific sentence
    :param data_container:          (h5py)  Container of the whole data, h5py object
    :param word_objects:            (h5py)  Container of all word data for a specific sentence
    ...
    if isinstance(available_objects[0], str):
        ...
        if "rawEEG" in available_objects:
            ...
            assert len(contentData) == len(etData), "different amounts of different data!!"
            zipped_data = zip(contentData, ffdData, gdData, trtData, psData, gptData, nfixData)
            word_level_data = {}
            word_idx = 0
            for word_obj, ffd, gd, trt, pS, gpt, nfix in zipped_data:
                word_string = load_matlab_string(data_container[word_obj[0]])
                if is_real_word(word_string):
                    data_dict = {}
                    data_dict["FFD"] = data_container[ffd[0]][0, 0] if len(data_container[ffd[0]].shape) == 2 else None
                    ...
                    data_dict["meanPupilSize"] = data_container[pS[0]][0, 0] if len(data_container[pS[0]].shape) == 2 else None
                    data_dict["GPT"] = data_container[gpt[0]][0, 0] if len(data_container[gpt[0]].shape) == 2 else None
                    data_dict["nFixations"] = data_container[nfix[0]][0, 0] if len(data_container[nfix[0]].shape) == 2 else None
                    #data_dict["SFD"] = data_container[sfd[0]][0, 0] if len(data_container[sfd[0]].shape) == 2 else None
                    #fixations_order_per_word.append(np.array(data_container[fixPos[0]]))
